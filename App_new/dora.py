...
# Definition (Article 3 defined terms)
definition = """
LOAD CSV WITH HEADERS FROM '$file_path' AS row
MERGE (d:Definition {id: row.id, regional_standard_regulation_id: 'DORA 2022/2554'})
ON CREATE SET
    d.label = row.label,
    d.name = row.name,
    d.legal_reference = row.legal_reference,
...
reg_to_definitions = """
MATCH (reg:RegionalStandardAndRegulation {regional_standard_regulation_id: 'DORA 2022/2554'})
MATCH (d:Definition {regional_standard_regulation_id: 'DORA 2022/2554'})
MERGE (reg)-[:REGULATION_CONTAINS_DEFINITION]->(d);
"""

definitions_to_article = """
MATCH (d:Definition {regional_standard_regulation_id: 'DORA 2022/2554'})
MATCH (a:Article {id: 'ART_03', regional_standard_regulation_id: 'DORA 2022/2554'})
MERGE (a)-[:ARTICLE_DEFINES_TERM]->(d);
"""
...
client.query(reg_to_definitions)
time.sleep(2)

client.query(definitions_to_article)
time.sleep(2)
