...
others, so scope by `framework_id` or the `:DTOPCatalogNode` anchor. It merges
two source files, `risk-catalog.json` and `control-catalog.json`, which state
...
`:DTOPCatalogNode`/`:DTOPCatalogRoot` to the latter.
