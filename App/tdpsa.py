...
ON CREATE SET
    ...
    cr.extension_date = CASE WHEN row.extension_date = '' THEN null ELSE date(row.extension_date) END,
    cr.response_date = CASE WHEN row.response_date = '' THEN null ELSE date(row.response_date) END,
    cr.response_method = CASE WHEN row.response_method = '' THEN null ELSE row.response_method END,
    cr.free_request_count = toInteger(row.free_request_count),
    cr.fee_charged = toBoolean(row.fee_charged),
    cr.completion_status = CASE WHEN row.completion_status = '' THEN null ELSE toBoolean(row.completion_status) END,
    cr.consumer_satisfied = CASE WHEN row.consumer_satisfied = '' THEN null ELSE toBoolean(row.consumer_satisfied) END,
    cr.appeal_submitted = toBoolean(row.appeal_submitted),
    cr.appeal_deadline = CASE WHEN row.appeal_deadline = '' THEN null ELSE date(row.appeal_deadline) END;
"""
