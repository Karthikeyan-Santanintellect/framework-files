...
# CORRECTED: Standardized constraints to use 'IS_framework_standard_id' consistently.
constraints = """
CREATE CONSTRAINT nist_IS_framework_standard_id IF NOT EXISTS FOR (f:Framework) REQUIRE f.IS_framework_standard_id IS UNIQUE;
CREATE CONSTRAINT nist_function_composite_id IF NOT EXISTS FOR (fn:Function) REQUIRE (fn.IS_framework_standard_id, fn.function_id) IS UNIQUE;
CREATE CONSTRAINT nist_category_composite_id IF NOT EXISTS FOR (c:Category) REQUIRE (c.IS_framework_standard_id, c.category_id) IS UNIQUE;
CREATE CONSTRAINT nist_subcategory_composite_id IF NOT EXISTS FOR (s:Subcategory) REQUIRE (s.IS_framework_standard_id, s.subcategory_id) IS UNIQUE;
"""
...
# CORRECTED: Changed property from 'id' to 'IS_framework_standard_id' for consistency.
IS_framework_and_standards = """
MERGE (f:ISFrameworksAndStandard {IS_framework_standard_id: "NIST_CSF_2.0"})
ON CREATE SET
...
MERGE (fn:Function {function_id: row.Function_Code, IS_framework_standard_id: "NIST_CSF_2.0"})
ON CREATE SET
...
categories = """
LOAD CSV WITH HEADERS FROM '$file_path' AS row
MERGE (c:Category {category_id: row.Category_Code, IS_framework_standard_id: "NIST_CSF_2.0"})
ON CREATE SET
...
MATCH (f:ISFrameworksAndStandard {IS_framework_standard_id: 'NIST_CSF_2.0'})
MATCH (fn:Function {IS_framework_standard_id: 'NIST_CSF_2.0'})
...
MATCH (fn:Function {IS_framework_standard_id: 'NIST_CSF_2.0'})
MATCH (c:Category {IS_framework_standard_id: 'NIST_CSF_2.0'})
...
MATCH (c:Category {IS_framework_standard_id: 'NIST_CSF_2.0'})
MATCH (s:Subcategory {IS_framework_standard_id: 'NIST_CSF_2.0'})
